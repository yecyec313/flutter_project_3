...
class _CartScreenState extends State<CartScreen> {
  ...
  void initState() {
    super.initState();
    Global.enumListener.addListener(enumChange);
    AuthRepository.authChangeNotifier.addListener(authChangeNotifierListener);
  }

  void enumChange() {
    if (Global.enumListener.value == Enumproduct.yes) {
      // setState(() {
      cartBloc?.add(CartStarted(AuthRepository.authChangeNotifier.value));
      // });
      Global.enumListener.value = Enumproduct.no;
    } else if (Global.enumListener.value == Enumproduct.yes2) {
      cartBloc?.add(CartStarted(AuthRepository.authChangeNotifier.value));
  ...
  @override
  void dispose() {
    Global.enumListener.removeListener(enumChange);
          ...
          child: BlocBuilder<CartBloc, CartState>(
            builder: (context, state) {
              if (state is CartLoading) {
                ...
                Global.enumproduct = Enumproduct.no;
                return EmptyView(
                    message:
